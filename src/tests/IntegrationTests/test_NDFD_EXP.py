...
from DataIngestion.DI_Classes.NDFD_EXP import NDFD_EXP

load_dotenv()


@pytest.mark.parametrize("source, series, location, interval, datum", [
    ("NDFD_EXP", "pAirTemp", "SBirdIsland", None, None),
    ("NDFD_EXP", "pAirTemp", "lagunamadre", timedelta(seconds=3600), None),
    ("NDFD_EXP", "pAirTemp", "SBirdIsland", timedelta(seconds=10800), None)
])
def test_ingest_series(source: str, series: str, location: str, interval: timedelta, datum: str):
    ...
    ingestSeries = NDFD_EXP()
    resultsSeries = ingestSeries.ingest_series(seriesDescription, timeDescription)
    print(f'RESULT: {resultsSeries}')
    print(f'Data: {resultsSeries.data}')
