...
"""This is a database migration script that will initialize version
    2.2 of the database. The change from version 2.1 to 2.2 is that
    we are adding rows to the locations reference table and the location
    mapping reference table for Beto Estrada's Virginia Key FL models. 
 """ 
#----------------------------------
# 
#
...
#Constants
CSV_FILE_PATHS = './tools/DatabaseMigration/2_2/init_data'


class Migrator(IDatabaseMigration):

    def update(self, databaseEngine: Engine) -> bool:
        """This function updates the database to version 2.2 which adds rows to the locations reference 
        ...
        self.insert_ref_data(self.readInitCSV('virginiakeyLocations.csv'), ref_dataLocation)
        self.insert_ref_data(self.readInitCSV('virginiakeyMapping.csv'), ref_dataLocation_dataSource_mapping)
    ...
    def rollback(self, databaseEngine: Engine) -> bool:
        """This function rolls the database back to version 2.1 which involves removing the changes 
           associated with version 2.2.
        ...
        fileNames = ['virginiakeyLocations.csv']
