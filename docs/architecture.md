...
Semaphore uses a set of shared data classes to standardize how information is passed between components. Rather than passing raw dictionaries or DataFrames, each component communicates using these objects.

### Series

The `Series` class is the primary data object used throughout Semaphore. It combines:

- A description of the dataset (`SeriesDescription` or `SemaphoreSeriesDescription`)
- A `TimeDescription`
- A pandas DataFrame containing the actual data

Input data gathered by the `DataGatherer` and predictions generated by the `ModelRunner` are both represented as `Series` objects.

### SeriesDescription

Describes an input dataset without containing the data itself.
...
Describes a prediction generated by Semaphore.
...
Defines the time range represented by a dataset.
...
Specifies any data integrity operations that should be applied before validation.

### Standard DataFrames

Semaphore uses two standard DataFrame formats:
...
| `dataUnit` | Measurement unit |
| `timeVerified` | Time the data was verified |
...
| Column | Description |
|---------|-------------|
| `dataValue` | Prediction values |
| `dataUnit` | Output unit |
| `timeGenerated` | Time the prediction was generated |
...
Database schema changes are handled through the migration system located in `DatabaseMigration/`.
See database.md for more information.

---
## Semaphore API

Semaphore provides a REST API built with FastAPI that allows users and applications to retrieve input data, model predictions, and prediction statistics.

The API acts as the interface between Semaphore and downstream applications such as dashboards or visualization tools.

### Responsibilities

- Retrieve input data from supported data sources.
- Retrieve the latest model predictions.
- Retrieve historical model predictions.
- Retrieve prediction statistics.
- Serialize Semaphore objects into JSON responses.
...
|----------|-------------|
| `/input/` | Retrieves input data from a configured data source. |
| `/output_latest/` | Returns the latest prediction for one or more models. |
| `/output_time_span/` | Returns predictions within a specified time range. |
| `/output_statistics/` | Returns stored statistics for one or more models. |
| `/health` | Returns the current health status of the API. |

### Serialization

The API converts internal `Series` objects into JSON before returning responses. Separate serializers are used for:
